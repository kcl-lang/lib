...
bool checkreturn pb_write(pb_ostream_t* stream, const pb_byte_t* buf, size_t count)
{
    if (count > 0 && stream->callback != NULL) {
        if (stream->bytes_written + count < stream->bytes_written || stream->bytes_written + count > stream->max_size) {
...
static bool checkreturn encode_array(pb_ostream_t* stream, pb_field_iter_t* field)
{
...
#endif
    {
        for (i = 0; i < count; i++) {
            ...
            if (PB_ATYPE(field->type) == PB_ATYPE_POINTER && (PB_LTYPE(field->type) == PB_LTYPE_STRING || PB_LTYPE(field->type) == PB_LTYPE_BYTES)) {
                bool status;
                void* pData_orig = field->pData;
                ...
                if (!field->pData) {
                    ...
                    status = pb_encode_tag_for_field(stream, field) && pb_encode_varint(stream, 0);
                } else {
...
static bool checkreturn pb_enc_bytes(pb_ostream_t* stream, const pb_field_iter_t* field)
{
    ...
    if (PB_ATYPE(field->type) == PB_ATYPE_STATIC && bytes->size > field->data_size - offsetof(pb_bytes_array_t, bytes)) {
