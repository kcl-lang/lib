 ...
 */
#if (defined(__GNUC__) && ((__GNUC__ > 3) || (__GNUC__ == 3 && __GNUC_MINOR__ >= 4))) || (defined(__IAR_SYSTEMS_ICC__) && (__VER__ >= 9040001))
...
static bool checkreturn decode_static_field(pb_istream_t* stream, pb_wire_type_t wire_type, pb_field_iter_t* field)
{
    ...
    case PB_HTYPE_ONEOF:
        if (PB_LTYPE_IS_SUBMSG(field->type) && *(pb_size_t*)field->pSize != field->tag) {
            ...
            if (field->submsg_desc->default_value != NULL || field->submsg_desc->field_callback != NULL || field->submsg_desc->submsg_info[0] != NULL) {
...
static void initialize_pointer_field(void* pItem, pb_field_iter_t* field)
{
    if (PB_LTYPE(field->type) == PB_LTYPE_STRING || PB_LTYPE(field->type) == PB_LTYPE_BYTES) {
        *(void**)pItem = NULL;
...
static bool pb_field_set_to_default(pb_field_iter_t* field)
{
    ...
    if (PB_LTYPE(type) == PB_LTYPE_EXTENSION) {
        ...
        if (init_data) {
            if (PB_LTYPE_IS_SUBMSG(field->type) && (field->submsg_desc->default_value != NULL || field->submsg_desc->field_callback != NULL || field->submsg_desc->submsg_info[0] != NULL)) {
    ...
    while (stream->bytes_left) {
        ...
        if (PB_HTYPE(iter.type) == PB_HTYPE_REPEATED && iter.pSize == &iter.array_size) {
            if (fixed_count_field != iter.index) {
                 ...
                 */
                if (fixed_count_field != PB_SIZE_MAX && fixed_count_size != fixed_count_total_size) {
    ...
    /* Check that all required fields were present. */
    {
        ...
        if (req_field_count > 0) {
            ...
            if ((req_field_count & 31) != 0) {
                if (fields_seen.bitfield[req_field_count >> 5] != (allbits >> (uint_least8_t)(32 - (req_field_count & 31)))) {
                    PB_RETURN_ERROR(stream, "missing required field");
                }
            }
        }
...
#else
    *(uint32_t*)dest = ((uint32_t)u.bytes[0] << 0) | ((uint32_t)u.bytes[1] << 8) | ((uint32_t)u.bytes[2] << 16) | ((uint32_t)u.bytes[3] << 24);
#endif
    return true;
}
...
#else
    *(uint64_t*)dest = ((uint64_t)u.bytes[0] << 0) | ((uint64_t)u.bytes[1] << 8) | ((uint64_t)u.bytes[2] << 16) | ((uint64_t)u.bytes[3] << 24) | ((uint64_t)u.bytes[4] << 32) | ((uint64_t)u.bytes[5] << 40) | ((uint64_t)u.bytes[6] << 48) | ((uint64_t)u.bytes[7] << 56);
#endif
    return true;
}
#endif
